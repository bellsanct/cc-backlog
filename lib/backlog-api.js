...
class BacklogAPIClient {
  ...
  async getStatuses(projectIdOrKey) {
    return this.request('GET', `/projects/${projectIdOrKey}/statuses`);
  }
  ...
  // ========================================
  // Convenience Aliases (for command compatibility)
  // ========================================

  /**
   * Alias for getProjectList
   * @param {Object} [options]
   * @returns {Promise<Array>} List of projects
   */
  async getProjects(options) {
    return this.getProjectList(options);
  }

  /**
   * Alias for addIssueComment
   * @param {string|number} issueIdOrKey - Issue ID or key
   * @param {Object} data - Comment data
   * @returns {Promise<Object>} Created comment
   */
  async addComment(issueIdOrKey, data) {
    return this.addIssueComment(issueIdOrKey, data);
  }

  /**
   * Alias for countIssues (returns count object)
   * @param {string|number} projectId - Project ID
   * @returns {Promise<Object>} Issue count by status
   */
  async getIssueCount(projectId) {
    // Get issues grouped by status
    const issues = await this.getIssues({ projectId: [projectId] });
    ...
    issues.forEach(issue => {
      ...
      if (statusName.includes('open') || statusName.includes('新規')) {
        counts.open++;
      } else if (statusName.includes('progress') || statusName.includes('処理中')) {
        counts.inProgress++;
      } else if (statusName.includes('close') || statusName.includes('完了')) {
        counts.closed++;
      }
    });
  ...
  /**
   * Alias for getVersionMilestoneList
   * @param {string|number} projectIdOrKey - Project ID or key
   * @returns {Promise<Array>} List of versions
   */
  async getVersions(projectIdOrKey) {
    return this.getVersionMilestoneList(projectIdOrKey);
  }

  /**
   * Alias for addVersionMilestone
   * @param {string|number} projectIdOrKey - Project ID or key
   * @param {Object} data - Version data
   * @returns {Promise<Object>} Created version
   */
  async addVersion(projectIdOrKey, data) {
    return this.addVersionMilestone(projectIdOrKey, data);
  }
}
